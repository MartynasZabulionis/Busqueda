...
class MiPropiaCola(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        #sorted(self.A, key = lambda x: x.path_cost, reverse=False)
        self.A.sort(key = lambda x: x.path_cost, reverse=False)
		#self.A.sort(key = lambda var: var.path_cost)
...
class MiPropiaEuristicaCola(Queue):

    def __init__(self, problem):
        self.A = []
        self.start = 0
        self.problemilla = problem
    ...
    def extend(self, items):
        ...
        #sorted(self.A, key = lambda x: (x.path_cost + int(self.problemilla.h(x))), reverse=True)
        self.A.sort(key = lambda x: (x.path_cost + int(self.problemilla.h(x))), reverse=True)
		#self.A.sort(key = lambda var: var.path_cost)
